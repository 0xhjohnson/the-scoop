...
// begin comment implementation

function createComment(url, request) {
  // declare consts to clean up comment obj. below
  const requestComment = request.body && request.body.comment;
  const commentBody = requestComment && requestComment.body;
  const commentUsername = requestComment && requestComment.username;
  const commentArticleId = requestComment && requestComment.articleId;
  const response = {};

  if (
    commentBody &&
    database.users[commentUsername] &&
    database.articles[commentArticleId]
  ) {
    ...
    database.users[commentUsername].commentIds.push(commentId);
    database.articles[commentArticleId].commentIds.push(commentId);
...
function updateComment(url, request) {
  const id = Number(url.split('/').filter(segment => segment)[1]);
  const storedComment = database.comments[id];
  const requestComment = request.body && request.body.comment;
  const response = {};

  // ensure request has body and updated comment
  if (!id || !requestComment) {
    ...
    storedComment.body = requestComment.body || storedComment.body; // update
    response.body = { comment: storedComment };
    response.status = 200;
  }
  return response;
}
...
function deleteComment(url, request) {
  ...
  // comment exists
  if (storedComment) {
    database.comments[id] = null; // delete comments from the database

    const article = database.articles[storedComment.articleId];
    const articleComments = article && article.commentIds;
    const user = database.users[storedComment.username];
    const userComments = user && user.commentIds;

    articleComments.splice(articleComments.indexOf(id), 1); // delete article's comment ID's
    userComments.splice(userComments.indexOf(id), 1); // delete author's comment ID's
...
function upvoteComment(url, request) {
  const id = Number(url.split('/').filter(segment => segment)[1]);
  const username = request.body && request.body.username;
  let storedComment = database.comments[id];
  const response = {};

  // comment and user must exist
  if (storedComment && database.users[username]) {
    storedComment = upvote(storedComment, username);
    response.body = { comment: storedComment };
    response.status = 200;
  } else {
    response.status = 400;
...
function downvoteComment(url, request) {
  ...
  // comment and user must exist
  if (storedComment && database.users[username]) {
    storedComment = downvote(storedComment, username);
...
const fs = require('fs');
const yamlDbName = 'scoopDb.yaml'; // set to file name

function loadDatabase() {
  try {
    // load scoopDb.yaml with utf8 encoding
    let yamlDb = yaml.safeLoad(fs.readFileSync(yamlDbName, 'utf8'));
    database = yamlDb;
...
function saveDatabase() {
  fs.writeFile(yamlDbName, yaml.safeDump(database), err => {
    if (err) throw err;
  });
}
